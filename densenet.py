...
class densenet_fcn(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.training:
            output = torch.squeeze(output, 1)
            predict0 = torch.squeeze(predict0, 1)
            predict1 = torch.squeeze(predict1, 1)
            predict2 = torch.squeeze(predict2, 1)
            predict3 = torch.squeeze(predict3, 1)
            return [predict0, predict1, predict2, predict3, output]
...
class densenet_fc(nn.Module):
    def __init__(self, dataset, source=False):
        ...
        self.fc = nn.Linear(57344, self.output_height * self.output_weight)
...
class densenet_fcn_deconv(nn.Module):
    r"""Densenet fully convolution mode, based on
    `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`
    Args:
        growth_rate (int) - how many filters to add each layer (`k` in paper)
    ...
    def __init__(self, dataset, growth_rate=16, block_config=[4, 5, 7, 10, 12], last_layer=15,
                 num_init_features=48, bn_size=4, drop_rate=0.2, efficient=True,
                 constrain=False, source=False):
        super(densenet_fcn_deconv, self).__init__()
